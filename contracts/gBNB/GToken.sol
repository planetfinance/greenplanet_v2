...
/**
 * @title Planet's GToken Contract
 * @notice Abstract base for GTokens
 * @author Astronaut
 */
abstract contract GToken is EIP20Interface, ExponentialNoError, TokenErrorReporter, ReentrancyGuard {
    ...
    */
    ...
    function _onlyAdminValidAddress(address _address) private view {
        require(msg.sender==admin && _address!=address(0), "no admin priv or 0 address");
    }

    modifier onlyAdminValidAddress(address _address){
        _onlyAdminValidAddress(_address);
        _;
    }
    /**
     * @notice construct a new money market
     * @param addresses The address of the Gammatroller, InterestRateModel, iGamma
    ...
    constructor(address[] memory addresses,
        ...
        uint err = _setGammatroller(GammatrollerInterface(addresses[0]));
        require(err == NO_ERROR, "setting gammatroller failed");
        ...
        err = _setInterestRateModelFresh(InterestRateModel(addresses[1]));
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        if (allowed != 0) {
            revert TransferGammatrollerRejection(allowed);
        }
        ...
        if(isBoostDeprecated == false) {
            _updateUserAndTotalFactorsInternal(src, EIP20Interface(iGamma).balanceOf(src));
            _updateUserAndTotalFactorsInternal(dst, EIP20Interface(iGamma).balanceOf(dst));
        }
    ...
    function supplyRatePerBlock() external view returns (uint) {
        ...
        Exp memory underlying = mul_(Exp({mantissa: exchangeRateMantissa}), totalSupply);	
        Exp memory borrowsPer = Exp({mantissa: div_(mul_( mul_(totalBorrows, expScale), expScale), underlying.mantissa)});
        Exp memory oneMinusReserveFactor = sub_(Exp({mantissa: mantissaOne}), Exp({mantissa: reserveFactorMantissa}));	
        Exp memory supplyRate = mul_(mul_(Exp({mantissa: borrowRateMantissa}), oneMinusReserveFactor), borrowsPer);	
    ...
    function accrueInterest() virtual public returns (uint) {
        ...
        gammatroller.getOracle().validate(address(this));
    ...
    function mintFresh(address minter, uint mintAmount) internal {
        ...
        if (allowed != 0) {
            revert MintGammatrollerRejection(allowed);
        }
       ...
       /*Fail if checkTransferIn fails */	
        require(checkTransferIn(minter, mintAmount) == 0, "checkTransferIn failed");	
        ...
        uint mintTokens = div_(mintAmount, exchangeRate);
        ...
        if(isBoostDeprecated == false) {
            _updateUserAndTotalFactorsInternal(minter, EIP20Interface(iGamma).balanceOf(minter));
        }

        require(doTransferIn(minter, mintAmount) == NO_ERROR, "transfer in failed");
        ...
        // unused function
        // gammatroller.mintVerify(address(this), minter, actualMintAmount, mintTokens);
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal {
        ...
        uint allowed = gammatroller.redeemAllowed(address(this), redeemer, redeemTokens);
        if (allowed != 0) {
            revert RedeemGammatrollerRejection(allowed);
        }
        ...
        if(isBoostDeprecated == false) {
            _updateUserAndTotalFactorsInternal(redeemer, EIP20Interface(iGamma).balanceOf(redeemer));
        }
        ...
        */
        ...
        require(doTransferOut(redeemer, redeemAmount) == NO_ERROR, "doTransferOut failed");
        ...
        /* We call the defense hook */
        gammatroller.redeemVerify(address(this), redeemer, redeemAmount, redeemTokens);
    ...
    /**
      ...
      */
    function _setDiscountLevelInternal(PlanetDiscount newDiscountLevel) internal onlyAdminValidAddress(address(newDiscountLevel)) returns (uint) {
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal {
        ...
        if (allowed != 0) {
            revert BorrowGammatrollerRejection(allowed);
        }
       ...
       require(doTransferOut(borrower, borrowAmount) == NO_ERROR, "doTransferOut failed");
     ...
     * @param borrower the account with the debt being payed off
     * @param repayAmount the amount of undelrying tokens being returned, or -1 for the full outstanding amount
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint) {
        ...
        if (allowed != 0) {
            revert RepayBorrowGammatrollerRejection(allowed);
        }
         ...
         */
        require(doTransferIn(payer, repayAmountFinal) == NO_ERROR, "doTransferIn failed");
        ...
        /* We call the defense hook */
        // unused function
        // gammatroller.repayBorrowVerify(address(this), payer, borrower, vars.actualRepayAmount, vars.borrowerIndex);
     ...
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, GTokenInterface gTokenCollateral) internal nonReentrant {
        accrueInterest();
    
        uint error = gTokenCollateral.accrueInterest();
        if (error != NO_ERROR) {
        ...
        liquidateBorrowFresh(msg.sender, borrower, repayAmount, gTokenCollateral);
    }
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, GTokenInterface gTokenCollateral) internal returns (uint, uint) {
        ...
        if (allowed != 0) {
            revert LiquidateGammatrollerRejection(allowed);
        }
        ...
        if (gTokenCollateral.accrualBlockNumber() != block.number) {
        ...
        (uint amountSeizeError, uint seizeTokens) = gammatroller.liquidateCalculateSeizeTokens(address(this), address(gTokenCollateral), actualRepayAmount);
        require(amountSeizeError == NO_ERROR, "LIQUIDATE_GAMMATROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");
        ...
        if (address(gTokenCollateral) == address(this)) {
            ...
            require(gTokenCollateral.seize(liquidator, borrower, seizeTokens) == NO_ERROR, "token seizure failed");
        }
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal {
        ...
        uint allowed = gammatroller.seizeAllowed(address(this), seizerToken, liquidator, borrower, seizeTokens);
        if (allowed != 0) {
            revert LiquidateSeizeGammatrollerRejection(allowed);
        }
        ...
        if(isBoostDeprecated == false) {
            _updateUserAndTotalFactorsInternal(borrower, EIP20Interface(iGamma).balanceOf(borrower));
            _updateUserAndTotalFactorsInternal(liquidator, EIP20Interface(iGamma).balanceOf(liquidator));
        }
    ...
    function getBoostDeprecatedStatus() external view returns (bool){
        return isBoostDeprecated;
    }

    function getMarketData() external view returns (uint256, uint256){
        return (totalSupply, totalFactor);
    }

    function getUserData(address user) external view returns (uint256, uint256){
        return (accountTokens[user], userFactors[user]);
    }

    function updateUserAndTotalFactors(address user, uint256 iGammaBalanceOfUser) external {
        require(msg.sender == address(gammatroller));
        _updateUserAndTotalFactorsInternal(user,iGammaBalanceOfUser);
    }

    function deprecateBoost() external{
        require(msg.sender == address(gammatroller));
        isBoostDeprecated = true;
    }

    function _updateUserAndTotalFactorsInternal(address user, uint256 iGammaBalanceOfUser) internal {

        uint256 oldFactor = userFactors[user];
        uint256 newFactor = (accountTokens[user] * iGammaBalanceOfUser).sqrt();
        if (oldFactor != newFactor) {
            userFactors[user] = newFactor;
            totalFactor = totalFactor - oldFactor + newFactor;
        }
    }
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setPendingAdmin(address payable newPendingAdmin) external onlyAdminValidAddress(newPendingAdmin) returns (uint) {
        // Check caller = admin
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setGammatroller(GammatrollerInterface newGammatroller) public onlyAdminValidAddress(address(newGammatroller)) returns (uint) {
        // Check caller is admin
    ...
    function _reduceReserves(uint reduceAmount) external nonReentrant returns (uint) {
        require(reduceAmount>0, "amount is 0");
        accrueInterest();
    ...
    function _reduceReservesFresh(uint reduceAmount) internal returns (uint) {
        ...
        uint currentTotalDiscount = discountLevel.totalDiscountGiven(address(this));
        uint differenceDiscount = currentTotalDiscount - totalDiscountReceived;
        uint totalReservesPrev = totalReserves;
        totalReserves = totalReserves - reduceAmount - differenceDiscount;
        totalDiscountReceived = currentTotalDiscount;
        ...
        require(totalReserves <= totalReservesPrev, "totalReserves > totalReservesPrev");
        require(reduceAmount <= totalReservesPrev - differenceDiscount, "invalid reduce amount");
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setInterestRateModelFresh(InterestRateModel newInterestRateModel) internal onlyAdminValidAddress(address(newInterestRateModel)) returns (uint) {
    ...
    /**
     * @notice updates the iGammaAddress
     * @dev Admin function to update the iGammaAddress. This called by GammaTroller
     * @param _newiGammaAddress the new iGamma address
     */
    function _updateiGammaAddress(address _newiGammaAddress) external {
        require((msg.sender == admin || msg.sender == address(gammatroller)) , "only admin or gammaTroller may change this");
        iGamma = _newiGammaAddress;
        emit iGammaAddressUpdated(_newiGammaAddress);
    }
     ...
     */
    function checkTransferIn(address from, uint amount) virtual internal view returns (uint);
     ...
     */
    function doTransferOut(address payable to, uint amount) virtual internal returns (uint);
